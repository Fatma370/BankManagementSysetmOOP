...
public:
	...
	string FormatDate(string date, string format)
	{
		...
		int day = format.find("dd");
		format.replace(day, 2, vDate[0]);
		int month = format.find("MM");
		format.replace(month, 2, vDate[1]);
		int year = format.find("yyyy");
		format.replace(year, 4, vDate[2]);
		return format;
	...
	{
		int day = format.find("dd");
		format.replace(day, 2,to_string(date.Day));
		int month = format.find("MM");
		format.replace(month, 2, to_string(date.Month));
		int year = format.find("yyyy");
		format.replace(year, 4, to_string(date.Year));
		return format;
	...
	static string  getCurrentTime() {
		...
		time_t t = time(0);
		tm* now = localtime(&t);
		string time = to_string(now->tm_hour) + ":" + to_string(now->tm_min) + ":" + to_string(now->tm_year);
		return time;
	}
