...
private:
    ...
    static stLoginInfo _getLoginInfoStruct(string LoginLine)
    {
        vector<string>vLoginInfo = clsString::Split(LoginLine, "#//#");
        stLoginInfo LoginInfo;
        LoginInfo.Time_date = vLoginInfo[0];
        LoginInfo.UserName = vLoginInfo[1];
        LoginInfo.PassWord =clsUtil::DecryptText( vLoginInfo[2],4);
        LoginInfo.Permissions = vLoginInfo[3];
        return LoginInfo;
...
public:
    ...
    static vector<stLoginInfo> getLoginRegisterList() {
        fstream Myfile;
        vector<stLoginInfo>vLoogins;
        ...
        if (Myfile.is_open()) {
            ...
            while (getline(Myfile, line))
            {
                vLoogins.push_back(_getLoginInfoStruct(line));
